...
Every run:
  ...
  2. Enforces the safety gates — sends NOTHING and reports PAUSED when:
       - trailing bounce rate >= 2% (only once the trailing window has
         >= 50 sends — single bounces on tiny early volumes don't halt
         the ramp; the dead address is already excluded from future draws)
       - ANY spam complaint in the trailing window (no minimum sample —
         complaints pause immediately)
...
BOUNCE_MIN_SAMPLE = 50       # bounce-rate gate needs >= 50 trailing sends
...
SUBJECT = "get your child's free personalized coloring page"
PREHEADER = "see your child's name in a custom coloring page"

HTML_TMPL = """<div style="font-family:Georgia,serif;max-width:560px;margin:0 auto;color:#1f2937">
<div style="display:none;max-height:0;overflow:hidden;opacity:0;">see your child's name in a custom coloring page</div>
<p>{greet}</p>
<p>you can create a free personalized coloring page for your child in just a few seconds. this is a great way to enhance their imagination and make learning more fun. with crayon kid, you can pick a theme, describe a scene, or turn a photo of their pet or toy into a coloring page.</p>
<p>for example, your child's name can star as the hero of a fairy tale, or as a brave knight on a quest. one parent told us: &quot;my child loves seeing their name in the stories &mdash; it's like they're part of the adventure.&quot;</p>
<p>to get started, create your child's first free coloring page:<br><a href="https://go.mehyar.us/LxOTQ2Co">create your child's first free page &rarr;</a><br>or copy and paste this link into your browser if you prefer:<br>https://go.mehyar.us/LxOTQ2Co</p>
<p style="font-size:13px;color:#6b7280">why you're getting this: your email was on a list of parents interested in kids' activities. if that's not you anymore, you can <a href="{unsub}">unsubscribe instantly</a> &mdash; one tap, gone forever.</p>
<p>&mdash; Mehyar, Crayon Kid</p>
</div>"""

TEXT_TMPL = """{greet}

you can create a free personalized coloring page for your child in just a few seconds. this is a great way to enhance their imagination and make learning more fun. with crayon kid, you can pick a theme, describe a scene, or turn a photo of their pet or toy into a coloring page.

for example, your child's name can star as the hero of a fairy tale, or as a brave knight on a quest. one parent told us: "my child loves seeing their name in the stories — it's like they're part of the adventure."

to get started, create your child's first free coloring page:
https://go.mehyar.us/LxOTQ2Co
or copy and paste it into your browser if you prefer.

why you're getting this: your email was on a list of parents interested in kids' activities. if that's not you anymore, you can unsubscribe instantly (one tap, gone forever): {unsub}
...
def main(argv: list[str]) -> int:
    ...
    rows = cf_d1("SELECT campaign_day, date, sent_count, bounce_count, complaint_count FROM warmup_campaign_daily WHERE brand=? ORDER BY campaign_day",
                 [BRAND])
    ...
    gate = None
    if t_sent >= BOUNCE_MIN_SAMPLE and bounce_rate >= BOUNCE_PAUSE_RATE:
        gate = f"bounce_rate {bounce_rate:.2%} >= 2% over last {len(trail)} day(s) ({t_sent} trailing sends)"
